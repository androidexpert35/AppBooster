...
class Spake2PlusHandler(
    ...
    // M = Hash-to-curve("M")
    // N = Hash-to-curve("N")
    // Values from BoringSSL's crypto/spake25519/spake25519.c (P256 variant)
     ...
     * @throws CryptoException If the peer's key is invalid.
     */
     ...
     * Uses HKDF with a specific info string.
     */
    private fun deriveKeyMaterial(rawSecret: ByteArray): ByteArray {
        val hkdf = HKDFBytesGenerator(SHA256Digest())
        // Use empty salt and "SPAKE2 secret" as info
    ...
    private fun hashToPoint(data: ByteArray): ECPoint {
        // Simple hash-to-curve: hash data, use as x, find valid y
        var hash = sha256(data)
        var attempt = 0

        while (attempt < 256) {
            try {
                val x = BigInteger(1, hash).mod(curve.field.characteristic)
                // Try to create a point with this x coordinate
                // y² = x³ + ax + b (for P-256: a = -3, b = specific value)
                val point = curve.createPoint(x, computeY(x))
                if (point.isValid) {
            ...
            hash = sha256(hash + attempt.toByte())
            attempt++
        }

        throw CryptoException("Failed to hash to curve point")
    }

    private fun computeY(x: BigInteger): BigInteger {
        ...
        val y2 = x3.add(ax).add(b).mod(p)

        // Square root mod p (p ≡ 3 mod 4 for P-256)
